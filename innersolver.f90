...
!subroutine evaldSD(n,m,l,u,d,lambda,JF,inform,bi)
subroutine evaldSD(n,m,l,u,d,lambda,JF,inform)
	...
	! ARRAY ARGUMENTS
	real(kind=8), intent(in)  :: l(n),u(n),JF(m,n)
	real(kind=8), intent(out) :: d(n),lambda(m)
	
	if ( m == 2 ) then
		call evaldSDbi(n,d,lambda,JF,inform)
	else
		call evaldSDmc(n,m,l,u,d,lambda,inform)
	end if
...
subroutine evaldSDbi(n,d,lambda,JF,inform)
	...
	! ARRAY ARGUMENTS
	real(kind=8), intent(in)  :: JF(2,n)
	real(kind=8), intent(out) :: d(n),lambda(2)	
	
	! LOCAL SCALARS
	integer      :: i
	real(kind=8) :: a1,a2,b,f0,f1,ft,tol,lambdatrial
	...
	lambdatrial = ( a2 - dot_product( JF(1,:), JF(2,:) ) ) / b**2
	
	if ( lambdatrial <= 0.0d0 .or. lambdatrial >= 1.0d0 ) then
		if ( f1 <= f0 ) then
			lambda(1) = 1.0d0
			lambda(2) = 0.0d0
			d = - JF(1,:)
		...
		ft = 0.5d0 * norm2( lambdatrial * JF(1,:) + ( 1.0d0 - lambdatrial ) * JF(2,:) )**2
		...
		elseif ( i == 2 ) then
			...
			lambda(1) = lambdatrial
			lambda(2) = 1.0d0 - lambdatrial
			d = - ( lambda(1) * JF(1,:) + lambda(2) * JF(2,:) )
			return
		end if
		
	end if

end subroutine evaldSDbi

! ******************************************************************
! Multi-criteria
! ******************************************************************

subroutine evaldSDmc(nvar,nf,lC,uC,d,multLg,inform)
