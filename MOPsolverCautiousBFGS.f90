subroutine cautiousBFGS(n,m,x,l,u,scaleF,checkder,outiter,time,nfev,ngev,theta,inform)

	use globals , only: sF,nfinner,xinner,d,JF,B,quadratic
	use myproblem
	...
	integer :: ind,j,mquad,mnquad,infoIS,infoLS,nfevLS,ngevLS,infofun,allocerr,&
	           maxoutiter,informfg,outiterLS,totiniterLS
	real(kind=8) :: stp,stpmin,stpmax,epsopt,ftol
	...
	real(kind=8) :: g(n),lambda(m),xprev(n),Fx(m),JFprev(m,n),phi0(m),gphi0(m)
  ...
  ! March of 2020.
	...
	allocate(d(n),xinner(n),sF(m),quadratic(m),JF(m,n),B(n,n,m),stat=allocerr)
	...
	Main_loop: do
		...
		do ind = 1,m
			...
			if ( infofun /= 0 ) then
				...
				deallocate(d,xinner,sF,quadratic,quadind,nquadind,JF,B)
		...
		if ( outiter == 0 ) then
			...
			call cbfgsupdate(n,m,x,xprev,JF,JFprev,theta,B)
		end if
			
		! Compute the search direction
		
		call evaldN(n,m,d,lambda,infoIS)
		...
		if ( outiter == 1 ) then 
			do ind = 1,m
				call sevalf(n,x,phi0(ind),ind,infofun)
				nfev = nfev + 1   
		...
		! Compute the stepsize satisfying the Armijo condition

			
		call armijo(evalphi,stp,stpmin,m,phi0,gphi0,Fx,ftol,iprintLS,  &
		            nfevLS,infoLS) 
	...
	1000 format(/,1X,'-------------------------------------------------------------------------------',/,1X,& 
					 '            Cautious BFGS method for Multiobjective Optimization               ',/,1X,& 
					 '-------------------------------------------------------------------------------')
