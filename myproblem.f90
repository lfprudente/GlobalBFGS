...
! ----------------------------------------------------------------------
	...
	if ( problem == 'DTLZ5' ) then 
		...
		theta(1) = x(1)
		a = pi / ( 4.0d0 * ( 1.0d0 + faux ) )
		do i = 2,m-1
			theta(i) = a * ( 1.0d0 + 2.0d0 * faux * x(i) )
		end do
		
		f = 1.0d0 + faux					
		do i = 1,m-ind
			f = f * cos( theta(i) * pi / 2.0d0 )
		end do
		
		if ( ind > 1 ) f = f * sin( theta(m-ind+1) * pi / 2.0d0 )
...
!		end do
		...
		allocate(theta(m-1),thetader(m-1))
		...
		do i = 1,m-ind
			do j = 1,m-ind
				if ( j == i ) then
					g(i) = - g(i) * pi / 2.0d0 * thetader(i) * sin( theta(i) * pi / 2.0d0 )
				else
					g(i) = g(i) * cos( theta(j) * pi / 2.0d0 )
				end if
			end do
			if ( ind > 1 ) g(i) = g(i) * sin( theta(m-ind+1) * pi / 2.0d0 )
		end do
		
		if ( ind > 1 ) then
			g(m-ind+1) = faux * pi / 2.0d0 * thetader(m-ind+1) * cos( theta(m-ind+1) * pi / 2.0d0 )
			do j = 1,m-ind
				g(m-ind+1) = g(m-ind+1) * cos( theta(j) * pi / 2.0d0 )
			end do
		end if
		...
		faux = 1.0d0
		do i = 1,m-ind
			faux = faux * cos( theta(i) * pi / 2.0d0 )
		end do

		if ( ind > 1 ) faux = faux * sin( theta(m-ind+1) * pi / 2.0d0 )
		
		do i = m,n
			g(i) = faux * 2.0d0 * ( x(i) - 0.5d0 )
		end do
		
		deallocate(theta,thetader)
